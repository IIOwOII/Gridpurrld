...
class Gridworld_ItemCollect():
    ...
    def action(self, a):
        ...
        reward  = (np.sqrt(np.power(self.max_x, 2) + np.power(self.max_y, 2)) - np.sqrt(np.power(self.x - self.item_x, 2) + np.power(self.y - self.item_y, 2))) / 10 - 2 
        if self.item == 1:
            reward += 20
        done    = self.is_done()

        return np.array([self.x, self.y, self.item_x, self.item_y, self.item]), reward, done
    ...
    def get_state(self):
        return np.array([self.x, self.y, self.item_x, self.item_y, self.item])
...
class RepalyBuffer():
    ...
    def sample(self, n):
        ...
        return torch.tensor(s_list, dtype = torch.float), torch.tensor(a_list), torch.tensor(r_list), torch.tensor(s_prime_list, dtype = torch.float)
...
def main(learning_rate = .0005, print_interval = 20, n_epi = 1000, trunc = 100000, epsilon_min = .01, epsilon_start = .08, epsilon_step = 200, epsilon_grad = .01, min_memory = 2000, device = torch.device('cpu')):
